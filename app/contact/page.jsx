  ...
  return (
    <div className='flex mx-12 lg:flex-row flex-col justify-center my-12 items-center'>
      ...
      <div className='bg-white  border-1 rounded-sm p-12 border-[#C5C6CD]'>
        <form ref={form} onSubmit={sendEmail} action="" className='flex gap-8 flex-col'>
          <div className='flex flex-col justify-center'>
            <label htmlFor="" className='text-lg '>Full Name</label>
            <input placeholder='John Doe' name='user_name' className={`border-1 p-2 rounded-sm border-[#C5C6CD] outline-none`} type="text" />
          </div>
          <div className='flex flex-col'>
            <label className={`text-lg`}  htmlFor="">Email</label>
            <input name="user_email" type="email" placeholder='jhondoe@gmail.com' className={`border-1 rounded-sm p-2 border-[#C5C6CD] outline-none`} name='user_email' />
          </div>
          <div className='flex flex-col'>
            <label className={`text-lg `} htmlFor="">Message</label>
            <textarea name="user_message"  placeholder='enter your comment here...' className='p-3 outline-none border-1 max-h-40 border-[#C5C6CD] rounded-sm' rows={6} id=""></textarea>
          </div>
          <div className='flex justify-center w-full'>
            <motion.button type='submit'  whileHover={{scale:1.05}} whileTap={{scale:0.95}}  className='outline-none bg-[#4B41E1] cursor-pointer gap-5 flex justify-center items-center text-white text-2xl w-full py-4 rounded-md'>Send Message <span><SendHorizonal color='white' /></span></motion.button>
          </div>
